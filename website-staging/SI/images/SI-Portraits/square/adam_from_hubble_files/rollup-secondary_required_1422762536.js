...
if(TS.boot_data.feature_email_integration){TS.templates.message_file_email_share=TS.templates.compile("message_file_email_share")
...
}if(TS.boot_data.feature_flexpane_rework){var c=["channel_page_details","channel_page_member_tabs","channel_page_member_lists","channel_page_member_row","channel_page_shared_files"];
...
return a
},buildMsgHTML:function(J,f){if(f){TS.dir(0,J)
}try{var g=true;
var I=J.msg;
if(false&&I.text){I=TS.utility.clone(I);
I.text+=" <slack-action://BSLACKBOT/help/files/D026MK7NF|testing>"
}var L=J.model_ob;
var B=J.prev_msg;
var R=!!J.highlight;
var s=!!J.no_attachments;
var G=!!J.standalone;
var x=!!J.full_date;
if(TS.model.prefs.fuller_timestamps&&!x){x=!TS.utility.date.sameDay(TS.utility.date.toDateObject(I.ts),new Date())
}var z=J.jump_link;
var h=!!J.starred_items_list;
var n=(J.container_id)?"#"+J.container_id:"";
var t=!!J.enable_slack_action_links;
var A=J.theme;
if(!A){A=TS.model.prefs.theme
}var y="";
var M=TS.members.getMemberById(I.user);
var p=true;
var c=!G;
var T=false;
var C=false;
var O=TS.utility.date.toDateObject(I.ts);
var j=false;
var K=false;
var e=!!(I.rsp_id);
var q=false;
var H=I.user;
var l=I.is_ephemeral;
if(!H&&g){H=TS.templates.builders.getBotIdentifier(I)
}var v;
if(B){v=(B.subtype=="file_comment"&&B.comment)?B.comment.user:B.user;
if(!v&&g){v=TS.templates.builders.getBotIdentifier(B)
}}if(!I.no_display&&!G){if(B){var D=TS.utility.date.toDateObject(B.ts);
if(L.last_read<=B.ts){C=true
}if(I.subtype&&I.subtype=="file_comment"&&I.comment){H=I.comment.user
}if(TS.utility.msgs.automated_subtypes.indexOf(I.subtype)!=-1){c=true;
p=true
}else{if(v==H&&TS.utility.msgs.automated_subtypes.indexOf(B.subtype)===-1){if(!I.subtype&&B.subtype&&B.subtype=="file_comment"){c=true;
p=true
}else{if(A=="light"&&I.subtype=="file_share"||I.subtype=="file_mention"){c=true
}else{c=false
}if(!B.subtype||(TS.templates.builders.getBotIdentifier(B)&&g)){p=false
}if(TS.utility.msgs.isTempMsg(I)&&(I.type=="bot_message"||I.user=="USLACKBOT")){p=true
}}}}if(!e&&!TS.utility.date.sameDay(O,D)){if(!$(n+' div.day_divider[data-date="'+TS.utility.date.toCalendarDate(I.ts)+'"]').length){try{y+=TS.templates.messages_day_divider({ts:I.ts})
}catch(Q){TS.logError(Q,"Problem with TS.templates.messages_day_divider 1.1 msg.ts:"+(I?I.ts:"no msg?"))
}}j=true;
var w=$(n+" div.day_divider");
if(w.length>0){var u;
var b=$(w[w.length-1]);
if(b.length){u="";
try{u=TS.templates.messages_day_divider({ts:b.data("ts")})
}catch(Q){TS.logError(Q,"Problem with TS.templates.messages_day_divider 2.1 $last_divider.data('ts'):"+b.data("ts"))
}b.replaceWith(u)
}if(w.length>1){var P=$(w[w.length-2]);
if(P.length){u="";
try{u=TS.templates.messages_day_divider({ts:P.data("ts")})
}catch(Q){TS.logError(Q,"Problem with TS.templates.messages_day_divider 3.1 $second_last_divider.data('ts'):"+P.data("ts"))
}P.replaceWith(u)
}}}}if(!e&&TS.utility.date.distanceInMinutes(O,D)>TS.model.msg_activity_interval){K=true;
L.last_time_divider=O
}}else{if(!$(n+' div.day_divider[data-date="'+TS.utility.date.toCalendarDate(I.ts)+'"]').length){try{y+=TS.templates.messages_day_divider({ts:I.ts})
}catch(Q){TS.logError(Q,"Problem with TS.templates.messages_day_divider 4.1 msg.ts:"+(I?I.ts:"no msg?"))
}}K=true;
L.last_time_divider=O
}}if(K){p=true;
T=true
}if(I.type!="message"){p=true
}if(I.subtype=="bot_message"){if(TS.templates.builders.getBotIdentifier(I)){if(!g){p=true
}}else{p=false
}}if(I.subtype=="me_message"||(B&&B.subtype=="me_message")){p=true;
c=true
}var S=true;
if(G){S=false
}var E=TS.utility.msgs.getMsgActions(I);
var a=false;
if(E.edit_msg||E.delete_msg){a=true
}var d=false;
if(I.file&&I.subtype=="file_share"||I.subtype=="file_mention"){var r=TS.files.getFileActions(I.file);
if(Object.keys(r).length>0){d=true
}}var i=TS.utility.msgs.isMessageUserHidden(I);
var F=false;
if(i){if(v==H){return""
}F=true
}var o={msg:I,actions:E,show_actions_cog:a,show_file_actions_cog:d,member:M,show_user:p,hide_user_name:q,show_divider:c,first_in_block:T,unread:C,unprocessed:e,highlight:R,model_ob:L,do_inline_imgs:S,msg_dom_id:TS.templates.makeMsgDomId(I.ts),standalone:G,full_date:x,jump_link:z,show_resend_controls:I.ts in TS.model.display_unsent_msgs,starred_items_list:h,theme:A,no_attachments:s,is_ephemeral:l,enable_slack_action_links:t,minimal_view:F};
if(!TS.utility.msgs.isTempMsg(I)){o.permalink=TS.utility.msgs.constructMsgPermalink(L,I.ts)
}var N;
if(I.subtype=="file_share"||I.subtype=="file_mention"){if(!I.file){}else{o.file=I.file;
o.is_mention=(I.subtype=="file_mention");
o.lightbox=false;
if(I.file.thumb_360_w==360||I.file.thumb_360_h==360){o.lightbox=true
}o.show_retina_thumb=false;
if(TS.model.is_retina&&I.file.thumb_720){o.show_retina_thumb=true
}if(I.subtype=="file_share"&&I.upload){o.show_initial_comment=true;
if(I.file.mode=="snippet"){y+=TS.templates.message_file_snippet_create(o)
}else{o.icon_class=TS.utility.getImageIconClass(I.file,"thumb_80");
try{y+=TS.templates.message_file_upload(o)
}catch(Q){var U=I.ts;
try{var m=TS.utility.clone(I);
delete m.text;
U+=" "+JSON.stringify(m,null,"\t")
}catch(k){}TS.logError(Q,"Problem with TS.templates.message_file_upload msg:"+U);
y+='<p class="small_top_margin small_bottom_margin"><code>Error rendering file_share msg</code></p>'
}}}else{if(I.file.user!=I.user){N=TS.members.getMemberById(I.file.user);
o.uploader=N
}if(I.file.mode=="snippet"){y+=TS.templates.message_file_snippet_share(o)
}else{if(I.file.mode=="post"){y+=TS.templates.message_file_post_share(o)
}else{if(I.file.mode=="space"){y+=TS.templates.message_file_space_share(o)
}else{if(I.file.mode=="email"){y+=TS.templates.message_file_email_share(o)
}else{o.icon_class=TS.utility.getImageIconClass(I.file,"thumb_40");
if(I.file.is_external){o.external_filetype_html=TS.templates.builders.makeExternalFiletypeHTML(I.file)
}y+=TS.templates.message_file_share(o)
}}}}}}}else{if(I.subtype=="file_comment"){if(B&&!B.no_display&&B.file&&I.file&&I.file.id==B.file.id){o.show_divider=false;
if(!j){o.is_file_convo_continuation=true
}}o.show_comment_quote_icon=true;
if(B&&!B.no_display&&B.file&&I.file&&I.file.id==B.file.id){if(B.subtype=="file_share"&&B.upload&&B.file.initial_comment){if(!j){o.show_comment_quote_icon=false
}}if(B.subtype=="file_comment"){if(!j){o.show_comment_quote_icon=false
}}}o.file=I.file;
o.icon_class=TS.utility.getImageIconClass(I.file,"thumb_40");
o.comment=I.comment;
M=(I.comment)?TS.members.getMemberById(I.comment.user):null;
o.member=M;
if(I.file&&I.file.user!=I.comment.user){N=TS.members.getMemberById(I.file.user);
o.uploader=N
}if(I.file&&I.file.mode=="post"){y+=TS.templates.message_file_post_comment(o)
}else{y+=TS.templates.message_file_comment(o)
}}else{y+=TS.templates.message(o)
}}y=y.replace(/\ue000/g,"").replace(/\ue001/g,"");
return y
}catch(Q){var U="";
if(I){U="msg.ts:"+I.ts;
delete J.model_ob;
try{J.msg=TS.utility.clone(I);
delete J.msg.text;
U+=" "+JSON.stringify(J,null,"\t")
}catch(k){}}TS.logError(Q,"Problem in buildMsgHTML with_args "+U);
return""
...
Handlebars.registerHelper("fileActionsEllipsis",function(e){return'<a class="file_actions fa fa-ellipsis-h" data-file-id="'+e.id+'"></a>'
});
